...
> 本插件当前同步主链路暂不使用该接口；仅作为上游能力参考。新增功能前必须先确认真实返回并更新主文档。
...
3. 查看阅读进度：调 `/book/getprogress`，展示阅读百分比和累计时长（**当前插件同步主链路暂不使用该接口，仅作为上游能力参考**）。
4. `chapterUid` 是后续查看章节划线热度（`/book/underlines`）和热门划线（`/book/bestbookmarks`）等接口的参数（**`/book/underlines` 当前插件同步主链路暂不使用，仅作为上游能力参考**）。
