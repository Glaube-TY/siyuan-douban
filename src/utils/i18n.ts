...
export function localizeKnownUiText(source: I18nSource, value: unknown): string {
    ...
    const exact: Record<string, [string, string]> = {
        ...
        "已设置为停止同步，跳过检测": ["syncSkippedIgnored", "已设置为停止同步，跳过检测"],
        "已设置为停止同步，未参与本次检测": ["syncSkippedIgnoredDetail", "已设置为停止同步，未参与本次检测"],
        ...
        "这是跳过项，如需同步请取消忽略。": ["syncSuggestionSkipped", "这是跳过项，如需同步请取消忽略。"],
        "这是跳过项，如需同步请取消忽略或使用强制同步。": ["syncSuggestionSkipped", "这是跳过项，如需同步请取消忽略。"],
