...
export async function updateEndBlocks(_plugin: any, blockID: string, wereadPositionMark: string, noteContent: any) {
    ...
    try {
        ...
        if (targetBlock) {
            ...
            const markBlockResult = await insertBlock("markdown", targetContent, undefined, lastBlock ? lastBlock.id : blockID);

            // 使用新插入的标记块作为目标块
            targetBlockID = markBlockResult[0].doOperations[0].id;
        }
